...
You can build your own image for an MCU with .NET nanoFramework. In most cases you'll use prebuilt images for popular boards, including advanced ESP32 devices like the ones from M5Stack for which we offer [specific firmware](https://github.com/nanoframework/Home#m5stack) images and NuGets with [Board Support Packages](https://github.com/nanoframework/nanoFramework.M5Stack).

However there are situations for which you'll want to build the image yourself. There are Developer Container images available for you to use. They have the toolchains and all the require tools. Checkout the article on [using Dev Container to build targets](https://docs.nanoframework.net/content/building/using-dev-container.html). In case you prefer not to use the Dev Containers, it's possible to use a local setup too. Every step well documented at [Building .NET nanoFramework](https://docs.nanoframework.net/content/building/index.html).
...
This is architected in such a way that the "foreign" code remains completely isolated from the CLR and the official libraries and drivers. With this, you can't break things upstream and it can be easily incorporated in the build process without requiring tweaks and handling complex merge operations.

Because the ultimate goal is to tap into this code from C#, there are two layers for this to happen:

* The C# library that acts as wrapper and offers the interface to the C/C++ code.
* The C/C++ code that gets called from C#.

Let's take look how this works by creating a very simple Interop library that reads a hardware ID and performs a "complex" calculation.
...
Navigate to C# .NET nanoFramework folder and select a Class Library project type.
...
* The property `HardwareSerial` has a only a getter because we are only reading the serial from the hardware. As that can’t be written, it doesn’t make sense providing a setter, right?
...
And this is all that’s required on the managed side! Build the project and look at the project folder (using VS Code for example). This is how it will looks like after a successful build:

![Class Library project properties](../static/img/png/nanoframework-interop-native-view.png)

From the top to the bottom, you can identify in the bin folder (debug or release flavour depending on your build preference) the .NET library that will be referenced in other projects. Please note that besides the .dll file there is the .xml file (the one that will allows IntelliSense to it’s magic), the .pdb file and another one with a .pe extension.
...
You’ve probably have also noted that there are a couple of other files with a similar name but ending with _mshl. That's the marshaling files which are responsible to add validations, sanity checks and marshal the interface between the managed and native code. Those are to be left alone. Again DO NOT change them!
...
Remember that here we are not in the C# world anymore where the CRL and Visual Studio take care of the hard stuff for us. In C++ things are very different! On this particular example if the caller wouldn’t have reserved the required 12 bytes in memory to hold the complete serial array, when writing onto it the 12 bytes from the serial could be overwriting something that is stored in the memory space ahead of the argument address. For types other than pointers such as bytes, integers and doubles this check is not required.
...
A few comments about the argument type (`CLR_RT_TypedArray_UINT8`). It acts like a wrapper for the memory block that holds the array (or a pointer if you prefer). The class for that type provides a function – called `GetBuffer()` – that returns the actual pointer that allows direct access to it. We need that because we have to pass a pointer when calling `memcpy`. This may sound a bit complicated, granted. If you have curiosity on the implementation details or want to know how it works I suggest that you delve into the .NET nanoFramework interpreter code in our [GitHub repository](https://github.com/nanoframework/nf-interpreter) and take a look at all this.
...
Visual Studio was kind enough to add there the code for the return value so we can start coding on top of that. Actually that has to be exactly there otherwise this code wouldn’t even compile. 😉
...
The next file to get our attention is `FindINTEROP-NF.AwesomeLib.cmake`. .NET nanoFramework uses CMake as it's build system. Skipping through the technical details, suffice to say that, as far as CMake is concerned, the Interop assembly will be treated as a CMake module and, because of that, the file name for the file, in order to have it properly included in the build, has to be named FindINTEROP-NF.AwesomeLib.cmake and has to be placed inside the CMake\Modules folder.
...
Please refer to the documentation mentioned above about using Dev Containers or setting up the local toolchain in order to build .NET nanoFramework firmware image.

Let's assume If you are using the CMake Tools module to build inside VS Code. You need to declare that you want this Interop assembly added to the build. Do so by opening the CMakeUserPresets.json file and navigate to the settings for the target you want it added to.
...
The following task is launching the image build. Fingers crossed that you wont’ get any errors… 😉
...
The content above describes the key aspects and steps to get Interop working. It is a complex feature and you can read a more through and detailed description of all this in [this blog post](https://jsimoesblog.wordpress.com/2018/06/19/interop-in-net-nanoframework/).

Also you can find the code related with Interop in .NET nanoFramework in our [samples repository](https://github.com/nanoframework/Samples/tree/main/samples/Interop).
